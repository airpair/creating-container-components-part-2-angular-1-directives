...
![Empty ot-site container, labeled](https://content-na.drive.amazonaws.com/cdproxy/templink/iaubtyeI-HFVCoeI5jBZ0Ap4FNwC6Jr49H4VCXNTXyMLAYspN?viewBox=1440)
...
If you set transclude to true, Angular will save a clone of the user's content before adding in the directive template. That way, it won't be overwritten and will be available to add to the DOM during the linking phase.
...
But we're not done.  We still need to add the corresponding labels on the directive template side - "transclude-id" attributes.  This way, when a component user defines "head" as the destination, we can find an exact match.
